...
MODO_PRUEBAS = True  # Ponlo en False para que el horario sea real
...
def esta_abierto():
    ...
    if not futuros:
        return False, "Ya hemos cerrado por hoy. ¡Te esperamos en el próximo turno! 🕗"
    return True, ""
...
def añadir_al_carrito(update: Update, context: CallbackContext, categoria, producto_id, cantidad):
    """Añade productos al carrito con check de horario"""
    query = update.callback_query
    query.answer()
    ...
    if not abierto:
        query.edit_message_text(
            f"🚫 **LO SENTIMOS**\n\n{motivo}\n\nNo aceptamos pedidos ahora mismo, pero puedes seguir mirando la carta.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 VOLVER A LA CARTA", callback_data='menu_principal')]]),
            parse_mode='Markdown'
        )
        return
...
def handle_message(update: Update, context: CallbackContext):
    """Maneja mensajes de texto con respuesta automática si cerrado"""
    if context.user_data.get('esperando_direccion', False):
        procesar_direccion(update, context)
    else:
        # Check de Horario para respuesta automática
        abierto, motivo = esta_abierto()
        if not abierto:
            update.message.reply_text(
                f"👋 ¡Hola! Actualmente la cocina está cerrada.\n\n{motivo}\n\nPuedes ver nuestra carta usando /menu, pero no aceptamos pedidos ahora mismo. 🍕",
                parse_mode='Markdown'
            )
        else:
...
def keep_alive():
    url = "https://pizzeria-bot-l4y4.onrender.com"
    time.sleep(10)
    while True:
        try:
